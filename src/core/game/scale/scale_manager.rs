...
impl ScaleManager {
    ...
    pub fn can_place_on_scale(&self, scale_id: usize, card: &Card) -> bool {
        let Some(scale) = self.scales.get(scale_id) else {
            return false
        };
    ...
    pub fn place_on_scale(&mut self, scale_id: usize, card: Card) -> PlayResult {
        let Some(scale) = self.scales.get(scale_id) else {
        ...
        PlayResult::ScalePlaced {
            scale_id,
            completed: self.scales[scale_id].is_complete(),
        }
    }

    /// Player wants to open a new scale (usually with an Ace).
    /// Only succeeds if the card is an Ace.
    pub fn open_scale(&mut self, card: Card) -> PlayResult {
        if card.value != 1 {
            return PlayResult::DoesNotFit;
        }
        ...
        PlayResult::ScaleOpened { scale_id }
    }
...
mod tests {
    ...
    fn open_scale_with_non_ace_fails() {
        let mut mgr = ScaleManager::new();
        assert_eq!(mgr.open_scale(card(5)), PlayResult::DoesNotFit);
    ...
    fn open_multiple_scales_increments_ids() {
        let mut mgr = ScaleManager::new();
        assert_eq!(mgr.open_scale(card(1)), PlayResult::ScaleOpened  { scale_id: 0 });
        assert_eq!(mgr.open_scale(card(1)), PlayResult::ScaleOpened  { scale_id: 1 });
        assert_eq!(mgr.open_scale(card(1)), PlayResult::ScaleOpened  { scale_id: 2 });
    ...
    fn place_on_nonexistent_scale_fails() {
        let mut mgr = ScaleManager::new();
        assert_eq!(mgr.place_on_scale(0, card(2)), PlayResult::DoesNotFit);
        assert!(mgr.scales.is_empty());
    }
    ...
    fn place_invalid_card_fails() {
        ...
        assert_eq!(mgr.place_on_scale(0, card(5)), PlayResult::DoesNotFit);
    ...
    fn place_on_correct_scale_among_many() {
        ...
        assert_eq!(result, PlayResult::ScalePlaced { scale_id: 1, completed: false });
    ...
    fn place_completes_scale_and_flags_completion() {
        ...
        for v in 2..=11 {
            assert_eq!(
                ...
                PlayResult::ScalePlaced { scale_id: 0, completed: false }
            );
        }
        // Queen (12) should complete the scale
        let result = mgr.place_on_scale(0, card(12));
        assert_eq!(result, PlayResult::ScalePlaced { scale_id: 0, completed: true });
    ...
    fn place_on_completed_scale_fails() {
        ...
        assert_eq!(mgr.place_on_scale(0, card(1)), PlayResult::DoesNotFit);
        assert_eq!(mgr.place_on_scale(0, card(5)), PlayResult::DoesNotFit);
    }
    ...
    fn king_wildcard_placed_on_non_empty_scale() {
        ...
        assert!(matches!(mgr.place_on_scale(0, card(4)), PlayResult::ScalePlaced { .. }));
    }
