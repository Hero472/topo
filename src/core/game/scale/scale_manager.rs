...
impl ScaleManager {
    ...
    pub fn place_on_scale(
        ...
        let scale = self.scales.get(scale_id).ok_or(MoveError::DoesNotFit)?;
        if !scale.accepts(&card) {
...
mod tests {
    ...
    fn open_multiple_scales_increments_ids() {
        let mut mgr = ScaleManager::new();
        assert_eq!(mgr.open_scale(card(1)), Ok(MoveSuccess::ScaleOpened { scale_id: 0 }));
        assert_eq!(mgr.open_scale(card(1)), Ok(MoveSuccess::ScaleOpened { scale_id: 1 }));
        assert_eq!(mgr.open_scale(card(1)), Ok(MoveSuccess::ScaleOpened { scale_id: 2 }));
    ...
    fn place_on_nonexistent_scale_fails() {
        let mut mgr = ScaleManager::new();
        assert_eq!(mgr.place_on_scale(0, card(2)), Err(MoveError::DoesNotFit));
        assert!(mgr.scales.is_empty());
    }
    ...
    fn place_completes_scale_and_flags_completion() {
        ...
        for v in 2..=11 {
            assert_eq!(
                ...
                Ok(MoveSuccess::ScalePlaced { scale_id: 0, completed: false })
            );
        }
        // Queen (12) should complete the scale
        let result = mgr.place_on_scale(0, card(12));
        assert_eq!(result, Ok(MoveSuccess::ScalePlaced { scale_id: 0, completed: true }));
    ...
    fn place_on_completed_scale_fails() {
        ...
        assert_eq!(mgr.place_on_scale(0, card(1)), Err(MoveError::DoesNotFit));
        assert_eq!(mgr.place_on_scale(0, card(5)), Err(MoveError::DoesNotFit));
    }
    ...
    fn king_wildcard_placed_on_non_empty_scale() {
        ...
        assert_eq!(mgr.place_on_scale(0, card(4)), Ok(MoveSuccess::ScalePlaced { scale_id: 0, completed: false }));
    }
